...
pub fn get_zed_workspaces() -> Result<Vec<Workspace>> {
    ...
    if !zed_db_path.exists() {
        debug!("Zed database directory does not exist: {}", zed_db_path.display());
    ...
    for channel in ZED_CHANNELS {
        ...
        if !channel_path.exists() {
            debug!("Zed channel directory does not exist: {}", channel_path.display());
            continue;
        }
        ...
        info!("Found Zed database for channel '{}': {}", channel, db_file.display());
        
        match get_workspaces_from_db(&db_file, channel) {
            Ok(mut workspaces) => {
                info!("Found {} workspaces in Zed channel '{}'", workspaces.len(), channel);
                all_workspaces.append(&mut workspaces);
            }
            Err(e) => {
                warn!("Failed to read workspaces from Zed database {}: {}", db_file.display(), e);
            }
        }
    }
...
fn get_workspaces_from_db(db_path: &PathBuf, channel: &str) -> Result<Vec<Workspace>> {
    ...
    let workspace_rows = stmt.query_map([], |row| {
        Ok((
            ...
            row.get::<_, Option<i64>>(7)?,  // remote_connections.port
